...
This scripts get filtered tsv with pairwise distances from the previous stage and produces: 
  * scatterplots along the chromosome with individual PC axes as Y
    coordinate and cluster ID as a color
...
# PC's to retain
N_PCA = 100
# Which PC to plot against genomic coordinate
OUT_PC = 1
...
c = pd.read_csv(
    jn(DATA_DIR, f"filtered_{chrom}.tsv"), sep="\t", index_col=[0, 1, 2], header=[0, 1]
)
# then concatenate others to it
for chrom in chr_ends["chr"][1:]:
    in_file = jn(DATA_DIR, f"filtered_{chrom}.tsv")
    c = c.append(pd.read_csv(in_file, sep="\t", index_col=[0, 1, 2], header=[0, 1]))
...
pd.merge(DF_index_values, DF_red_data, on="lin_coord", how="left").to_csv(
    f"all_chr_PCA{N_PCA}.csv", sep=",", index=False
)
...
for i, j in combinations([1, 2, 9, 6, 8], 2):
    ...
    plt.clf()
    plt.scatter(
        reduced_data[:, i], reduced_data[:, j], marker=".", s=2, c=labels, cmap="Set1"
    )
    plt.savefig(f"coords_all_chr_PC{i}_PC{j}_PCA{N_PCA}.png")
...
plt.clf()
plt.scatter(
    y=reduced_data[:, 2], x=coords, marker=".", s=2, c=chrom_n, cmap=custom_cmap
)
...
plt.clf()
plt.scatter(
    y=reduced_data[:, 1], x=coords, marker=".", s=2, c=chrom_n, cmap=custom_cmap
)
...
plt.clf()
plt.scatter(
    y=reduced_data[:, 0], x=coords, marker=".", s=2, c=chrom_n, cmap=custom_cmap
)
...
for cl_n in range(2, 10):
    ...
    plt.clf()
    plt.scatter(
        y=reduced_data[:, OUT_PC], x=coords, marker=".", s=2, c=labels, cmap="Set1"
    )
    prev_x = 0
