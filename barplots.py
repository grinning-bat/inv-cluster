...
genome_index = pd.read_csv(
    IND_FILE,
    sep="\t",
    header=None,
    names="contig,contig_len,location,basesPerLine,bytesPerLine".split(","),
)
regular_chr = genome_index.contig.apply(
    lambda x: x in [f"chr{i}" for i in range(1, 26)]
)
...
for cl_n in range(2, CL_MAX + 1):
    ...
    if PLOT_ALL:
        ax.scatter(
            y=out, x=coords, marker=".", s=1, c=labels, cmap=ccm, vmin=0, vmax=ccm.N
        )
        ax.set_ylim([0.5, 11])
        ax.set_yticks([0, 8])
    else:
