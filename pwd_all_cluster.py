...
for chrom in chr_ends['chr'][1:]:
    ...
    c = c.append(
                   pd.read_csv(in_file, sep='\t', index_col=[0,1,2], header=[0,1])
                )
...
pd.merge( DF_index_values,
          DF_red_data,
          on='lin_coord',
          how='left' ).to_csv(f'all_chr_PCA{N_PCA}.csv', sep=',', index=False )
