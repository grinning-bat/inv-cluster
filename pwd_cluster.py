...
window_pwd = pd.read_csv(
    jn(DATA_DIR, f"window_pwd.ops.{chrom}.100000.100000.diploid.tsv"),
    sep="\t",
    index_col=[0, 1],
    header=[0, 1],
)
...
if DRAW_LINES:
    ...
    x_lines = list(
        bound[["inv_start_left", "inv_start_right", "inv_end_left", "inv_end_right"]]
    )
...
for i in coords:
    accessible.append(
        vp.get_accessible_size(bed_fn, chrom, start=i - 50000, end=i + 50000)
    )

# here is filtered set of df rows
# TODO add here column of coords, to avoid separate filtration of coords list
accessible_fixed = pd.DataFrame(
    [row for row, cov in zip(window_pwd.iloc, accessible) if cov > THR]
)
...
plt.clf()
plt.scatter(
    reduced_data[:, 0], reduced_data[:, 1], marker=".", s=2, c=coords, cmap="viridis"
)
...
# labels = db.labels_
plt.scatter(
    reduced_data[:, 0], reduced_data[:, 1], marker=".", s=2, c=labels, cmap="Set1"
)
...
accessible_fixed.loc[:, ("linear", "coord")] = [
    i[1] + chr_start for i in accessible_fixed.index
]
